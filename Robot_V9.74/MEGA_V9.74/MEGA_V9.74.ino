...
#define LCD_KEYPAD
#define TFT				// jdp - can use both at once
//#define PIXHAWK					// Uncomment if using pixhawk
//#define STOP_BTN					// Uncomment if using the stop button over interrupt pin
...
/*  JDP redoing this part to support both LCD and TFT
...
#define Microswitch_2  46               // Define Pin 46 on the MEGA to detect the microswitch  (Bumper LH or Lift Top End Stop)
	...
	// JDP change all motor pins
	//Motor A
	#define ENAPin 7                // EN Pins need a digital pin with PWM
	#define IN1Pin 23               // IN Pins dont need digital PWM
	#define IN2Pin 22
	//Motor B
	#define ENBPin 6                // EN Pins need a digital pin with PWM
	#define IN3Pin 25               // IN Pins dont need digital PWM
	#define IN4Pin 26  
	...
	#if defined(PIXHAWK)
	#define PIXHAWK_LH_PWM 4    // MEGA D11
	#define PIXHAWK_RH_PWM 5    // MEGA D12
	#endif
	...
	#if defined(PIXHAWK)
	#define PIXHAWK_LH_PWM 11        // MEGA D8
	#define PIXHAWK_RH_PWM 12         // MEGA D9
	#endif
...
int  Rain_Total_Hits_Go_Home        = 20; //EEPROM            // This sensor only makes sense in combination with a mower docking station
// as the mower is sent there to get out of the rain.
//Battery Settings
float Battery_Max               = 29.4;                       // Max battery volts in Volts. 3S = 12.6V
float Battery_Min               = 21.0;   //EEPROM            // Lower Limit of battery charge before re-charge required.
byte  Low_Battery_Detected      = 0;                          // Always set to 0
byte  Low_Battery_Instances_Chg = 21;     //EEPROM            // Instances of low battery detected before a re-charge is called..
float Amps_Charging_Setting     = 11.5;                        // Set value at which charge is detected in Amps (normally between 0.5 and 0.8)  0.5 = my 330/LAM   0.8 = my LM
...
int PWM_Slow_Speed_RH          = 160;                         // JDP changed from 175? Straight line speed when the mower is almost over the wire Right Wheel.
...
int PWM_Blade_Speed            = 255;     //EEPROM           // JDP changed from 250? PWM signal sent to the blade motor (speed of blade) new motor works well at 245.
...
void setup() {
	...
	Serial.println(F(" "));
	Serial.println(F(" "));  
	if (Robot_Type == 1) Serial.print(F("Pages T10 Lawnbot :"));		//jdp
	...
	//JDP not sure why this would set Serial3
	...
	if (Use_ESP01_for_basic_WIFI == 1) {      // JDP
		Setup_RemoteXY (); 
	}												
	...
	Setup_Gyro();
	delay(100);
	Setup_Relays();	//JDP dont know why this was not in the code was in 9.0
...
void loop() {
	...
	//check if remotexy is conneted if not skip all this *** added by JDP *****************
	Serial.print(F("connect flag outside loop = "));
	Serial.println(RemoteXY.connect_flag);
	
if ((Use_ESP01_for_basic_WIFI == 1) && (RemoteXY.connect_flag ==1)) 
{
	Main_RemoteXY_Loop ();
	//Serial.println ("");
	//Serial.print (" first_connected_flag =  ");
	//Serial.println(first_connected_flag);
...
// WIFI Commands from and to APP JDP added If WIFI_Enabled == 1
if ((WIFI_Enabled == 1) && (Manuel_Mode == 1))    Receive_WIFI_Manuel_Commands();
if ((WIFI_Enabled == 1) && (Manuel_Mode == 1))    Print_LCD_Info_Manuel();
